...
| Complete a QA/UAT pass, or otherwise change what testing exists for a module (add a new test type, close a coverage gap, discover a real gap) | Update `docs/testing-status-dashboard.html`'s matching module row(s) in §03 (chip level + cited evidence) and the §02 rollup scorecard in the same commit — the scorecard's numbers must stay arithmetically derived from §03, never typed independently. |
| Add, remove, or change a committed E2E/functional Playwright spec (`tests/e2e/`), or add a scenario worth tracking | Add/update its row in `docs/test-catalog.md` (stable `TC-<MODULE>-<NNN>` ID referenced in the spec's test title; flip its `Automated` column when a spec starts covering it). If the spec relies on new/changed QA identities, tenants, or seed data, update `docs/test-data-register.md` too. See ADR-0032 for the layer's scope (on-demand, not CI-gated) and the ID/category scheme. |
